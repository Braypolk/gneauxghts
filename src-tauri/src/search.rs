...
fn line_number_for_offset(paragraph: &IndexedParagraph, offset: usize) -> Option<usize> {
    paragraph
        .lines
        .iter()
        .find(|line| offset >= line.start_offset && offset <= line.end_offset)
        .map(|line| line.line_number)
        .or_else(|| paragraph.lines.first().map(|line| line.line_number))
}
