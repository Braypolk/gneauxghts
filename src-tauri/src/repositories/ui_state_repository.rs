...
//! lists, collapsed sets, forgotten-note metadata). Delegates to the
//! helpers in [`crate::state::persistence`].

use crate::state::{
    db_set_last_opened_note_id, db_set_note_collapsed, db_set_note_hidden, db_set_note_order,
    db_set_recent_note_ids, read_state, write_state, PersistedState,
};
use std::path::Path;
