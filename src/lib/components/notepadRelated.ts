...
export const MIN_NOTEPAD_WIDTH_FOR_SIDE_RELATED = 760;
...
export function computeRelatedDrawerLayout(
  notepadShell: HTMLDivElement,
  isRelatedPanelCollapsed: boolean,
  viewportWidth = window.innerWidth
): RelatedDrawerLayout {
  const shellRect = notepadShell.getBoundingClientRect();
  const drawerGap = 16;
  const rightBoundary = findHorizontalClipBoundary(notepadShell, viewportWidth) - 8;
...
export function getNotepadCardStyle(
