...
interface NotepadSessionControllerDeps {
  getTitle: () => string;
  ...
  applySessionSnapshot: (snapshot: NotepadSessionSnapshot) => void;
...
export function createNotepadSessionController({
  getTitle,
  getBodyMarkdown,
  getCurrentMarkdown,
  getCurrentPath,
  ...
  async function clearNotepad({ canRestore = true }: { canRestore?: boolean } = {}) {
    ...
    setForgottenNote(canRestore && hasContent ? createForgottenNote(draft, forgottenPath) : null);
    applySessionSnapshot(createEmptySessionSnapshot());
    setCanUnforget(canRestore && hasContent);
